"""Generic HTTP fetch backends — the app scrapes THROUGH these (normal scraping
only). Reused from the arxiv-eval design. Used for the arbitrary-site page +
on-page transcript scrape, and as the "plain vs impersonate" axis in the
stealth race. Each backend is available()-gated: a missing dep makes a
candidate SKIP, never crash.

Backends:
  urllib      -- stdlib baseline (no stealth). Trips bot detection on hardened targets.
...
class FetchOut:
    ...
    status: int = 0
    text: str = ""
    blocked: bool = False        # 403/429/challenge page / soft-block
    latency_ms: float = 0.0
    bytes_down: int = 0
    error: str = ""
    content: bytes = b""         # raw body (needed for binary/media targets)
...
def _blocked(status: int, text: str) -> bool:
    if status in (403, 429, 503):
        return True
    low = text[:2000].lower()
    if len(text) < 300 and "rate exceeded" in low:
        return True
    return any(s in low for s in (
        "captcha", "are you a robot", "verify you are human", "access denied",
        "unusual traffic", "confirm you're not a robot", "sign in to confirm"))
