"""Fetch backends the discovery/fetch candidates scrape THROUGH — normal
scraping only.

Each backend is available()-gated: a missing dep or unreachable service makes a
candidate SKIP, never crash (scrapler-eval convention). No Substack API/RSS
here — those are grader-only.

Backends:
  urllib      -- stdlib baseline (no stealth). Follows 301s (substack.com ->
                 custom-domain pubs) automatically.
  curl_cffi   -- TLS/JA3 impersonation (curl-impersonate engine). Stealth path.
...
class FetchOut:
    ...
    bytes_down: int = 0
    error: str = ""
    content: bytes = b""         # raw body (kept for binary targets)
    final_url: str = ""          # after redirects (substack -> custom domain)
...
def _blocked(status: int, text: str) -> bool:
    if status in (403, 429, 503):
        return True
    low = text[:3000].lower()
    # soft-blocks: HTTP-200 bodies that are actually challenge/limit pages
    if len(text) < 400 and ("rate" in low and "exceed" in low
                            or "too many requests" in low):
        return True
    return any(s in low for s in (
        "captcha", "are you a robot", "verify you are human", "access denied",
        "just a moment...", "cf-challenge", "attention required"))
    ...
    def get(self, url: str, timeout: int = 30) -> FetchOut:
        ...
        try:
            ...
            return FetchOut(True, r.status, txt, _blocked(r.status, txt), dt,
                            len(body), content=body, final_url=r.url)
    ...
    def get(self, url: str, timeout: int = 30) -> FetchOut:
        ...
        try:
            ...
            return FetchOut(True, r.status_code, txt,
                            ...
                            content=r.content, final_url=str(r.url))
        except Exception as e:
            dt = (time.perf_counter() - t0) * 1000
            return FetchOut(False, 0, "", False, dt, 0, error=str(e))
